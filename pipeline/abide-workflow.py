...
def get_subject(base_path, dataset, site):
    ...
    subjects = [s for s in os.listdir(datadir) if (s.startswith('sub-') 
        and os.path.isdir(s))]
    return subjects

@pydra.mark.task
def create_cmd(
    base_path, dataset, site,
    subject,
    fs_license,
    base_name='BASE',
    ...
    if not outdir_template:
        outdir_template = os.path.join(base_name, f'{dataset}-processed', site)
    ...
    --ignore slicetiming --cifti-output"
...
wf_inputs = {
    ...
    "fmriprep_image": "/om4/group/gablab/data/singularity-images/fmriprep-v1.3.0p2.sif",
    "pydra_cache": "/scratch/Thu/nlo/pydra_cache_dir"
}

sbatch_args = '-J abide-fmriprep -t 00:20:00 --mem=16GB --cpus-per-task=4'
...
wf = Workflow(name="wf", 
    ...
    **wf_inputs)
wf.add(get_subject(name='get_subject', base_path=wf.lzin.base_path,
    dataset=wf.lzin.dataset, site=wf.lzin.site))
wf.add(create_cmd(name='create_cmd', subject=wf.get_subject.lzout.out, 
    base_path=wf.lzin.base_path,
    dataset=wf.lzin.dataset, site=wf.lzin.site,
    fs_license=wf.lzin.fs_license).split('subject'))

wf.set_output([("out", wf.create_cmd.lzout.out)])

with Submitter(plugin='slurm', sbatch_args=) as sub:
    sub(wf)
